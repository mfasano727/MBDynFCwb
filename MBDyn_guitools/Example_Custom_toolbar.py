...
import FreeCAD, FreeCADGui, os, sys, math
...
#---------------------------------------------------------------------------
    ...
    def labelUi(self,title=translate("draft","Label"),callback=None):
        ...
        for s in ["Custom","Name","Label","Position","Length","Area","Volume","Tag","Material"]:
            combo.addItem(s)
        combo.setCurrentIndex(["Custom","Name","Label","Position","Length","Area","Volume","Tag","Material"].index("Custom"))
        l.addWidget(combo)
        ...
        if self.sourceCmd: 
            if (self.chooserButton.isVisible()):
                try:
                    ...
                    dialogDir = os.path.dirname(None) # reasonable default?
...
#                    fname = utf8_decode(fname[0])  # 1947: utf8_decode fails ('ascii' codec can't encode character)
    ...
    def getDefaultColor(self,type,rgb=False):
        ...
        if type == "snap":
            ...
            color = QtGui.QColor(746455039>>8)
            r = color.red()/255.0
            g = color.green()/255.0
            b = color.blue()/255.0
        else: 
...
if not hasattr(FreeCADGui,"draftToolBar2"):
    FreeCADGui.draftToolBar2 = DraftToolBar()
    FreeCADGui.draftToolBar2.Activated()
